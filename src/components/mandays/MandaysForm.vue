...
const handlePreview = async () => {
    try {
        const html = await getPreviewHtml('mandays');
  ...
  <div class="animate-fade-in relative">
    ...
    <div v-if="!isModalOpen" class="flex flex-col items-center justify-center py-20 gap-6">
        <div :class="['p-6 rounded-full shadow-2xl transition-transform hover:scale-110 duration-500 cursor-pointer group', 
            isDarkMode ? 'bg-black border-2 border-red-900 shadow-red-900/50' : 'bg-white border-2 border-blue-100 shadow-blue-200']"
            @click="isModalOpen = true">
            <span class="text-6xl group-hover:rotate-12 transition-transform block">{{ isDarkMode ? '💀' : '📄' }}</span>
        </div>
        <h2 :class="['text-3xl font-black uppercase tracking-widest text-center', isDarkMode ? 'text-red-600 font-cinzel drop-shadow-md' : 'text-blue-700 font-sans']">
            {{ isDarkMode ? 'Invoke Mandays' : 'Input Mandays' }}
        </h2>
        <p :class="['text-sm max-w-md text-center', isDarkMode ? 'text-red-900/80 font-serif italic' : 'text-blue-600/80']">
        ...
        <button @click="isModalOpen = true" 
            ...
            isDarkMode ? 'bg-red-900 text-white hover:bg-red-800 border border-red-950 font-cinzel tracking-widest' : 'bg-blue-600 text-white hover:bg-blue-700']">
            ...
            <div :class="['sticky top-0 z-10 flex items-center justify-between p-6 border-b backdrop-blur-md', 
                ...
                <div>
                    ...
                    <p :class="['text-[10px]', isDarkMode ? 'text-stone-500 italic' : 'text-slate-400']">
                        {{ isDarkMode ? 'The vertical oath.' : 'Fill clearly and correctly.' }}
                    </p>
                </div>
            ...
            <div class="p-6 space-y-8">
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between">
                        <h3 :class="['text-xs font-black tracking-[0.2em] uppercase', isDarkMode ? 'text-red-900 font-cinzel' : 'text-slate-400 font-sans']">
                            {{ isDarkMode ? 'I. Vessel Identity' : 'Data Karyawan' }}
                        </h3>
                        <div :class="['h-[1px] flex-1 ml-4', isDarkMode ? 'bg-gradient-to-r from-red-900/50 to-transparent' : 'bg-slate-200']"></div>
                    </div>
                    
                    <div :class="['p-6 space-y-8 border rounded-2xl shadow-sm transition-all text-sm', isDarkMode ? 'bg-neutral-950/50 border-red-900/30' : 'bg-white border-slate-100']">
                        <div class="space-y-3">
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-2 bg-neutral-900/0 rounded-lg p-1">
                                    <button @click="syncData" :disabled="isSyncing" 
                                        ...
                                        isDarkMode ? 'text-indigo-400 hover:bg-indigo-900/30' : 'text-blue-600 bg-blue-50 hover:bg-blue-100']">
                            ...
                            <div class="relative group">
                                ...
                                <select v-model="employee.name" :disabled="isAssigneeLoading" :class="[inputClass, 'pl-10 h-12', isAssigneeLoading ? 'opacity-50' : '']">
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                <label :class="labelClass">{{ isDarkMode ? 'Identification (NIK)' : 'NIK' }}</label>
                                <input v-model="employee.no" type="text" :class="[inputClass, 'h-10']" />
                            </div>
                            <div>
                                <label :class="labelClass">{{ isDarkMode ? 'Legion (Squad)' : 'Squad' }}</label>
                ...
                <div class="space-y-4">
                    <div class="flex items-center justify-between pb-2 border-b border-transparent">
                         <div class="flex items-center gap-4">
                            <h3 :class="['text-xs font-black tracking-[0.2em] uppercase', isDarkMode ? 'text-red-900 font-cinzel' : 'text-slate-400 font-sans']">
                                {{ isDarkMode ? 'II. Daily Rituals' : 'A. Daily Activity' }}
                            </h3>
                        ...
                        <button @click="addRegularRow" 
                            ...
                            isDarkMode ? 'bg-red-950 text-red-500 border border-red-900/50 hover:bg-red-900 hover:text-white' : 'bg-blue-500 text-white hover:bg-blue-600 hover:shadow-blue-200']">
                            <span>+</span> {{ isDarkMode ? 'INSCRIPTION' : 'Add Task' }}
                        </button>
                    </div>
                    ...
                    <TransitionGroup name="list" tag="div" class="space-y-4">
                         <div v-if="regularTasks.length === 0" key="empty"
                            ...
                            <p>{{ isDarkMode ? 'The parchment is blank. Add a ritual.' : 'Belum ada task.' }}</p>
                        </div>
                            ...
                            <div class="space-y-5">
                                <div class="flex gap-4">
                                    <div class="w-1/3 space-y-1">
                                         <label :class="labelClass">{{ isDarkMode ? 'Date' : 'Tanggal' }}</label>
                                    ...
                                    <div class="w-1/3 space-y-1">
                                         <label :class="labelClass">{{ isDarkMode ? 'Sigil (ID)' : 'Jira Ticket' }}</label>
                                         <input v-model="task.ticketNumber" @blur="autoFillLink(task)" type="text" :placeholder="isDarkMode ? 'ID...' : 'Ticket #'" :class="[inputClass, 'uppercase font-mono']" />
                                    </div>
                                    
                                    <div class="w-1/3 space-y-1">
                                         <label :class="labelClass">{{ isDarkMode ? 'Sigil Link' : 'Link Jira' }}</label>
                                         <input v-model="task.ticketLink" type="text" :placeholder="isDarkMode ? 'URL...' : 'https://...'" :class="[inputClass]" />
                                    </div>
                                </div>
                                    ...
                                    <label :class="labelClass">
                                     ...
                                     <div class="relative">
                                        <textarea v-model="task.description" rows="2" 
                                            :placeholder="isDarkMode ? 'Chant the description...' : 'Deskripsi pekerjaan...'" 
                                            :class="[inputClass, 'pr-12 transition-all group-focus-within/desc:ring-1', isDarkMode ? 'group-focus-within/desc:ring-red-900' : 'group-focus-within/desc:ring-blue-500']"></textarea>
                                        
                                        <button @click="enhanceDescription(index, 'regular')" 
                                            :class="['absolute top-2 right-2 p-1.5 rounded-lg transition-all active:scale-90', 
                                            isDarkMode ? 'hover:bg-red-900/30 text-red-500' : 'hover:bg-purple-100 text-purple-500']" title="AI Enhance">
                                            <svg v-if="enhancingId === `regular-${index}`" class="w-4 h-4 animate-spin" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
                ...
                <div class="space-y-4 pt-4 border-t border-dashed" :class="isDarkMode ? 'border-red-900/30' : 'border-slate-200'">
                    <div class="flex items-center justify-between pb-2">
                        <h3 :class="['text-xs font-black tracking-[0.2em] uppercase', isDarkMode ? 'text-red-900 font-cinzel' : 'text-slate-400 font-sans']">
                            {{ isDarkMode ? 'III. Nocturnal Sacrifices' : 'B. Overtime' }}
                        </h3>
                        <button @click="addOvertimeRow" 
                            ...
                            <span>+</span> {{ isDarkMode ? 'SACRIFICE' : 'OT' }}
                        </button>
                    </div>
                        ...
                        <div v-for="(task, index) in overtimeTasks" :key="index" 
                            ...
                            <div class="flex flex-wrap gap-4 md:flex-nowrap items-end">
                                <div class="w-full md:w-1/4 space-y-1">
                                     ...
                                     <input v-model="task.date" type="date" :class="[inputClass, 'w-full font-mono']" />
                                </div>
                                <div class="w-24 space-y-1">
                                     <label :class="['text-[10px] font-bold block mb-1 uppercase', isDarkMode ? 'text-stone-500' : 'text-slate-400']">Durasi</label>
                                     <input v-model="task.duration" type="number" step="0.5" :placeholder="isDarkMode ? 'Hrs' : 'Jam'" :class="[inputClass, 'w-full text-center font-mono']" />
                                </div>
                                <div class="flex-1 space-y-1">
                                     <label :class="['text-[10px] font-bold block mb-1 uppercase', isDarkMode ? 'text-stone-500' : 'text-slate-400']">Link</label>
                                     <input v-model="task.ticketLink" type="text" :placeholder="isDarkMode ? '...' : 'Link'" :class="[inputClass, 'w-full']" />
                                </div>
                            </div>
                            
                            <div class="mt-4 space-y-1 relative">
                                <label :class="['text-[10px] font-bold block mb-1 uppercase', isDarkMode ? 'text-stone-500' : 'text-slate-400']">
                                    {{ isDarkMode ? 'Sacrifice Details' : 'Keterangan Lembur' }}
                                </label>
                                <div class="relative">
                                    <input v-model="task.description" type="text" :placeholder="isDarkMode ? 'Details...' : 'Keterangan...'" :class="[inputClass, 'pr-10']" />
